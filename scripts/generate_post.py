...
TYPE_PROMPTS = {
    ...
    "news": "Сделай новостной пост для сообщества ВКонтакте в стиле короткой актуальной заметки про тренд на автоматизацию, ИИ и роботов в продвижении. Без выдуманных фактов и неподтверждённых цифр.",
...
def pick_post_type(posted):
    if not posted:
        return POST_TYPES[0]
    return POST_TYPES[len(posted) % len(POST_TYPES)]
...
def call_perplexity(prompt):
    ...
    if not api_key:
        raise RuntimeError("PERPLEXITY_API_KEY не найден")
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Ты создаёшь посты для сообщества ВКонтакте. Пиши на русском языке. "
...
def build_html(title, description, post, image_url, post_url):
    ...
    safe_post_url = html.escape(post_url)
...
<head>
  ...
  <meta property="og:url" content="{safe_post_url}">
